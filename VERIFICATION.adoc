...
`engine.mjs {plus} mission.mjs {plus} level.mjs {plus} ui.js` (imports/exports stripped);
`build.mjs` re-runs the witness on the _transformed_ core at build time.

[cols=",,,",options="header",]
|===
|# |Claim |Script |Result
|1 |Glider kinematics tables (4 headings × 4 phases, deltas {plus} lane invariant) match real B3/S23 evolution |verify1.mjs |derived empirically, round-trip checked
|2 |Gosper gun at (2,2) emits one isolated SE glider per 30 gens on lane 14; gun{plus}absorber population bounded over 1200 gens |verify2.mjs |PASS
|3 |Eater1 (sym 4) landing outcomes by approach lane λ |verify3.mjs |λ=0 CLEAN — the only clean lane; λ=−1..−5, {plus}1..{plus}5 fatal (flavors measured); λ≤−6 PASS
...
|10 |Reacquire cannot hijack ambient traffic: candidate gliders wholly present in the counterfactual shadow are rejected |engine.mjs findGlider(...,notInGrid) {plus} verify6 regression |PASS
...
glider and eater together, which preserves the measured _relative_ lane
