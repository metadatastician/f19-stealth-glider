...
| 3 | Eater1 (sym 4) landing outcomes by approach lane λ | verify3.mjs | λ=0 CLEAN — the only clean lane; λ=−1..−5, +1..+5 fatal (flavors measured); λ≤−6 PASS |
...
- Claim #3 was originally cited to a session transcript rather than to a
  runnable artefact. `verify3.mjs` now discharges it. That script (and
  `verify9.mjs`) additionally had to be fixed: both threw `stamp out of bounds`
  at the most negative approach lanes. The fix enlarges the grid and co-shifts
  glider and eater together, which preserves the measured *relative* lane
  because Life is translation-invariant away from borders — the fix is in the
  harness, never in `stamp`.
